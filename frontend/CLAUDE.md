...
2026-08-13에 대표 지시로 완전히 삭제했습니다 (커밋 `70e8eb8`). 코드·데이터·이미지가 모두 제거된 상태입니다.
...
* 2026-08-29에 **git 이력에서도 지웠습니다** (`git filter-repo` + force push).
  당시 최신 트리에서만 지워져 있었고, 공개 리포라 옛 커밋 주소로 팀원 사진 15장과
  고객사 로고 9개가 그대로 열리던 상태였습니다. 같은 작업으로 `backend/`(seed.py에
  같은 명단이 있었음)도 이력째 제거했습니다. 로컬 백업은 `P/iv-backups/20260829/`.
* 어떤 경로로도 **복구하지 않습니다.**
