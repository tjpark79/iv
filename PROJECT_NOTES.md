...
## 0-1. 절대 다시 올리지 않는 것 (2026-08-13)

**Our Partners(파트너 명단)와 「함께한 기업」(클라이언트 로고) 섹션은 사이트에 다시 만들지 않는다.**

2026-08-13에 대표 지시로 코드·데이터·이미지를 전부 삭제했다(커밋 `70e8eb8`).

2026-08-29에 **git 이력에서도 제거**했다. 8/13 삭제는 최신 트리에만 반영돼서,
공개 리포의 옛 커밋 주소로 사진 15장과 실명이 그대로 열리고 있었다. 게다가 이
파일과 `frontend/CLAUDE.md`가 옛 커밋 해시를 적고 있어 그 주소를 찾는 것도 어렵지
않았다. `git filter-repo`로 이력을 재작성하고, force push만으로는 GitHub 서버의 옛
객체가 남아 계속 열리는 것을 확인해 **리포를 삭제 후 재생성**했다. 같은 명단이 있던
`backend/`(FastAPI, 프론트가 호출하지 않는 죽은 코드)도 함께 제거했다.
로컬 백업은 `P/iv-backups/20260829/`에 있다.

- 삭제 대상: `Team.tsx`, `Clients.tsx`, `content.ts`의 `TEAM_MEMBERS`·`CLIENTS`와 `TeamMember`·`Client` 타입, `getTeam`·`getClients`, `public/team/`(사진 15장), `public/logos/`(로고 9개)
- 이미지까지 지운 이유: 페이지에서 섹션만 빼면 파일이 `public/`에 남아 링크 없이도 URL로 직접 접근된다. 실제로 `/team/shyoon.jpg`가 200을 내고 있었다. 제3자(외부 파트너·고객사)의 실명·소속·사진·로고가 동의 없이 계속 노출되는 상태였다.
- git 이력에는 남아 있으나 **복구하지 않는다.**
- 애드센스 대응으로 페이지 분량을 늘려야 할 때도 이 섹션의 부활을 검토하지 않는다. 회사 소개·일하는 방식·인사이트 글로 채운다.
- 푸터와 `/contact`·`/privacy`의 「박태준」(대표자, 개인정보 보호책임자)은 별개이며 법정 기재사항이므로 유지한다.
...
- `src/lib/content.ts`에 서비스/포트폴리오/타임라인 데이터가 배열로 직접 박혀있음 (팀·클라이언트는 위 0-1 항목대로 삭제됨)
